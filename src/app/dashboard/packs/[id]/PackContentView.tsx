  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white shadow-sm">
      ...
      <div
        ...
        className="-mx-px flex gap-1 overflow-x-auto border-b border-zinc-100 px-3 pt-3 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
      >
        {TABS.map((t) => (
          <button
            key={t}
...
function OverviewTab({ content, meta }: { content: PackContent; meta: Meta }) {
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm leading-relaxed text-zinc-700">{content.overview.summary}</p>

      {/* At a glance */}
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {stats.map((s) => (
          <div key={s.label} className="rounded-xl bg-violet-50/60 p-3 text-center ring-1 ring-inset ring-violet-100">
  ...
  return (
    <ol className="space-y-3">
      {content.lessonPlan.sections.map((s, i) => (
        <li key={i} className="flex gap-4 rounded-xl border border-zinc-100 bg-zinc-50/50 p-4">
          <div className="flex w-16 shrink-0 flex-col items-center justify-center rounded-lg bg-white text-center shadow-sm ring-1 ring-zinc-100">
            <span className="text-base font-extrabold text-violet-700">
              {s.durationMins ?? '—'}
            </span>
            <span className="text-[10px] font-medium uppercase text-zinc-400">min</span>
          </div>
          <div className="min-w-0">
            <p className="text-sm font-bold text-zinc-900">{s.title}</p>
            <p className="mt-0.5 text-sm text-zinc-600">{s.detail}</p>
          </div>
        </li>
      ))}
    </ol>
    ...
    <div className="grid gap-3 sm:grid-cols-2">
      {content.slides.map((slide, i) => (
        <div key={i} className="overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm">
          ...
          <div className="p-4">
            <p className="text-sm font-bold text-zinc-900">{slide.title}</p>
            ...
            {slide.teacherNotes ? (
              <p className="mt-3 rounded-lg bg-amber-50 px-2.5 py-1.5 text-xs italic text-amber-800">
                💡 {slide.teacherNotes}
              </p>
            ) : null}
          </div>
  ...
  return (
    <div className="space-y-4">
      {content.worksheets.map((w, i) => (
        <div key={i} className="rounded-xl border border-zinc-200 p-4 sm:p-5">
          <div className="mb-3 flex items-center gap-2">
            <Badge color={worksheetColor(w.level)}>{w.level}</Badge>
            <span className="text-sm font-bold text-zinc-900">{w.title}</span>
          </div>
          {w.intro ? <p className="mb-3 text-xs text-zinc-500">{w.intro}</p> : null}
          <ol className="list-decimal space-y-1.5 pl-5 text-sm text-zinc-700">
            {w.questions.map((q, j) => (
              <li key={j}>
                {q.prompt}
                {q.marks ? <span className="text-zinc-400"> ({q.marks} marks)</span> : null}
              </li>
            ))}
          </ol>
          <details className="group mt-3">
            <summary className="inline-flex cursor-pointer list-none items-center gap-1 rounded-lg bg-violet-50 px-3 py-1.5 text-xs font-semibold text-violet-700 [&::-webkit-details-marker]:hidden">
              Show answer key
      ...
      <div>
        ...
        <ol className="mt-2 list-decimal space-y-2 pl-5 text-sm text-zinc-700">
          {content.assessment.questions.map((q, i) => (
            <li key={i}>
              {q.prompt}
              <span className="font-medium text-zinc-400"> ({q.marks} marks)</span>
            </li>
          ))}
        </ol>
      </div>
      <div>
        ...
        <div className="mt-2 space-y-2">
          {content.markScheme.map((m, i) => (
            <div key={i} className="rounded-lg border border-zinc-100 bg-zinc-50/60 px-3 py-2 text-sm">
              <span className="font-bold text-violet-700">{m.questionRef}</span>{' '}
              <span className="text-zinc-600">{m.answer}</span>{' '}
              <span className="font-semibold text-zinc-400">[{m.marks}]</span>
            </div>
          ))}
        </div>
      </div>
...
function TeacherNotesTab({ content }: { content: PackContent }) {
  return (
    <div className="space-y-6">
      <NoteBlock title="Common misconceptions" accent="bg-rose-50 text-rose-700" items={content.teacherNotes.misconceptions} />
      <NoteBlock title="Key teaching points" accent="bg-violet-50 text-violet-700" items={content.teacherNotes.teachingPoints} />
...
function ExportTab() {
  return (
    <div className="space-y-5">
      ...
      <p className="rounded-xl bg-zinc-50 px-4 py-3 text-xs text-zinc-500 ring-1 ring-inset ring-zinc-100">
...
function ExportButton({ label, sub, icon }: { label: string; sub: string; icon: string }) {
  return (
    <button
      ...
      className="flex cursor-not-allowed items-center gap-3 rounded-xl border border-zinc-200 bg-zinc-50 p-4 text-left"
    >
      ...
      <span className="min-w-0">
        <span className="flex items-center gap-2 text-sm font-bold text-zinc-500">
          {label}
          <span className="rounded bg-zinc-200 px-1.5 py-0.5 text-[10px] font-bold uppercase text-zinc-500">
            Soon
          </span>
        </span>
...
function Detail({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl border border-zinc-100 bg-zinc-50/50 p-3">
      ...
      <p className="mt-0.5 whitespace-pre-line text-sm text-zinc-800">{value}</p>
    </div>
  );
}
