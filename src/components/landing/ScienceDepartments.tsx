  ...
  return (
    <section id="departments" className="py-16 sm:py-20">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="relative overflow-hidden rounded-3xl border border-violet-100 bg-white p-6 shadow-sm sm:p-10">
          ...
          <div className="relative grid gap-10 lg:grid-cols-2 lg:items-center">
            <div>
              ...
              <div className="mt-6 flex flex-wrap gap-2">
                ...
                {ROADMAP.map((r) => (
                  <span
                    key={r}
                    className="inline-flex items-center gap-1.5 rounded-full bg-zinc-50 px-3 py-1.5 text-xs font-medium text-zinc-500 ring-1 ring-inset ring-zinc-200"
                  >
                    <Dot className="bg-zinc-300" /> {r}
                    <span className="text-[10px] font-semibold uppercase text-zinc-400">soon</span>
                  </span>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <ShowcaseTile Icon={CellIcon} title="Biology" subtitle="IGCSE · GCSE · A-Level" accent="text-fuchsia-600 bg-fuchsia-50" live />
              <ShowcaseTile letters="Ed" title="Edexcel" subtitle="Spec-aligned content" accent="text-violet-600 bg-violet-50" live />
              <ShowcaseTile Icon={FlaskIcon} title="Chemistry" subtitle="On the roadmap" accent="text-cyan-600 bg-cyan-50" />
              <ShowcaseTile Icon={AtomIcon} title="Physics" subtitle="On the roadmap" accent="text-sky-600 bg-sky-50" />
            </div>
          </div>
        </div>
      </div>
...
function ShowcaseTile({
  ...
  return (
    <div className={`relative rounded-2xl border border-zinc-100 bg-white p-4 shadow-sm ${live ? '' : 'opacity-70'}`}>
      {live ? (
        <span className="absolute right-3 top-3 flex items-center gap-1 text-[10px] font-bold uppercase tracking-wide text-emerald-600">
